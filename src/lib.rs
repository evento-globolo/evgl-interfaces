...
pub source_url: String,
    ...
    if self.source.trim().is_empty() { return Err(ValidationError("source must not be empty".into())); }
    if self.venue.trim().is_empty() { return Err(ValidationError("venue must not be empty".into())); }
    if self.source_url.trim().is_empty() { return Err(ValidationError("source_url must not be empty".into())); }
            Ok(())
        }

...
`evgl-lib-core` implements these contracts natively and owns separate server-only definitions. `TrustedActor`, `ServerRequestContext`, and `InternalCommand` must never appear here or in `evgl-clients` artifacts.
